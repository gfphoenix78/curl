...
static CURLcode add_haproxy_protocol_header(struct connectdata *conn)
{
  ...
  Curl_dyn_init(&req_buffer, DYN_HAXPROXY);

  result = Curl_dyn_add(&req_buffer, proxy_header);
  if(result)
    return result;

  result = Curl_buffer_send(&req_buffer, conn, &conn->data->info.request_size,
                            0, FIRSTSOCKET);
      ...
      if(ptr) {
        ...
        if(*ptr || semicolonp) {
          ...
          if(conn->allocptr.host &&
            ...
            result = Curl_dyn_addf(req_buffer, "%s\r\n", compare);
          }
          if(semicolonp)
            free(semicolonp);
          if(result)
...
#define TE_HEADER "TE: gzip\r\n"
  ...
  /* GET/HEAD/POST/PUT */
  result = Curl_dyn_addf(&req_buffer, "%s ", request);
  if(result)
    return result;
  ...
  if(conn->bits.httpproxy && !conn->bits.tunnel_proxy) {
    ...
    result = Curl_dyn_add(&req_buffer, url);
    Curl_safefree(data->set.str[STRING_TEMP_URL]);
  }
  else
#endif
  if(paste_ftp_userpwd)
    result = Curl_dyn_addf(&req_buffer, "ftp://%s:%s@%s",
                           conn->user, conn->passwd,
                           path + sizeof("ftp://") - 1);
  else {
    result = Curl_dyn_add(&req_buffer, path);
    if(result)
      return result;
    if(query)
      result = Curl_dyn_addf(&req_buffer, "?%s", query);
  }
  if(result)
    return result;
  ...
  if(data->cookies || addcookies) {
    ...
    if(co) {
      ...
      while(co) {
        if(co->value) {
          if(0 == count) {
            result = Curl_dyn_add(&req_buffer, "Cookie: ");
            if(result)
              break;
          }
    ...
    if(addcookies && !result) {
      if(!count)
        result = Curl_dyn_add(&req_buffer, "Cookie: ");
      if(!result) {
        result = Curl_dyn_addf(&req_buffer, "%s%s", count?"; ":"",
                               addcookies);
        count++;
      }
    }
  ...
  case HTTPREQ_POST_MIME:
    ...
    if(conn->bits.authneg) {
      /* nothing to post! */
      result = Curl_dyn_add(&req_buffer, "Content-Length: 0\r\n\r\n");
      if(result)
        return result;
  ...
  case HTTPREQ_POST:
    ...
    if(!Curl_checkheaders(conn, "Content-Type")) {
      result = Curl_dyn_add(&req_buffer, "Content-Type: application/"
      ...
      if(conn->httpversion != 20 &&
        ...
        if(!data->req.upload_chunky) {
          ...
          if(postsize) {
            ...
            result = Curl_dyn_addf(&req_buffer, "%x\r\n", (int)postsize);
            if(!result) {
              result = Curl_dyn_addn(&req_buffer, data->set.postfields,
                                     (size_t)postsize);
              if(!result)
          ...
          if(!result)
            result = Curl_dyn_add(&req_buffer, "\x30\x0d\x0a\x0d\x0a");
          /* 0  CR  LF  CR  LF */
          included_body += 5;
        }
        if(result)
    ...
    /* issue the request */
    result = Curl_buffer_send(&req_buffer, conn, &data->info.request_size,
